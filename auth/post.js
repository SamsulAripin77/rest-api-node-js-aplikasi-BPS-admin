...
app.post("/postUser", (req, res) => {
    ...
    // { "deviceTokens": "kdjflkdffklsdfjlkdsfjlskf", "jabatan": "SKS",
    // "jabatanLengkap": "kepala Bps", "kodeWilayah": "3202", "nama": "subagya
    // sahrudin", "email": "sunarto@email.com", "password": "sunarkdlkjt",
    // "nip_atasan": "adminSipp", "KodeWilayahAtasan": "3202" }
    ...
    // =====================================================
    const deviceTokens = req.body.deviceTokens || 'devTok'
    var imageUrl = ""
                ...
                if (snapshot.exists()) {
                    ...
                    const cekAtasan = admin.database().ref(`/${kode}/user/`).orderByChild("nip").equalTo(nip_atasan).once('value', function (snapshot) {
                        if (snapshot.exists()) {
                            ...
                            // through each item found and print out the emails
                            snapshot.forEach(function (childSnapshot) {
                                ...
                                if (deviceTokens === undefined) {
                                    ...
                                    console.log("image default: https://firebasestorage.googleapis.com/v0/b/sistempenggansipres" + "ensi.appspot.com/o/Profile%2Fusers.png?alt=media&token=4fb53426-9874-464f-b814" + "-ec4e3177d677")
                                    ...
                                    admin.auth().createUser({
                                        ...
                                        uid = userRecord.uid
                                        imageUrl = "https://firebasestorage.googleapis.com/v0/b/sistempenggansipresensi.appspot.co" + "m/o/Profile%2Fusers.png?alt=media&token=4fb53426-9874-464f-b814-ec4e3177d677"
                                        ...
                                        ref.set({
                                            ...
                                            uidAtasan
                                        }).then(function (value) { // add user ke cek user
                                            ref1.set({username, password}).then(function (value) {
                                                const db3 = admin.database()
                                                const ref3 = db3.ref(`/${kodeWilayah}/user/${uid}`)
                                                ref3.once('value', function (snapshot) {
                                                    const data = snapshot.val()
                                                    console.log('+++++++++++++++data dibawah adalah data yang baru saja ditambahkan ke databasa' + 'e')
                                                    console.log(data)
                                                    res.send(data)
                                                }).catch((error) => {
